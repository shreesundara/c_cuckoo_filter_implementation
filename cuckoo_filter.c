...
struct __cuckoo_filter
{
   ...
   FINGER_PRINT_TYPE * __m_bit_finger_print_array;//The internal m_bit_array that contains the finger-print of the given keys. This is divided into multiple groups(buckets) containing max 8 elements or entries.
 
   M_BIT_ARRAY_ID_TYPE __total_no_of_buckets;//The total no.. of logical buckets into which the actual finger-print array is divided into. The index of these buckets should start from '1'. This is the assumption.
...
__insert_element(const char* name,const char* key)
{
    ...
    else
    {
        ...
        randomlyChosenBucket = (&bucket_2 ? (rand()%2) : &bucket_1) ;//This is either 1 or 2.
        ...
        for(cuckoo_kick_count = 1; cuckoo_kick_count <= __MAX_CUCKOO_KICKS; cuckoo_kick_count++)
        {
            ...
            randomlySelectedIndex = (rand()%__MAX_ELEMS_IN_A_BUCKET)+(__getFirstIndexOfTheBucket(filter,*(randomlyChosenBucket)));
