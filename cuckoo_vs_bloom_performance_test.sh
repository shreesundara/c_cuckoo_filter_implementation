...
CUCKOO_FILTERNAME="cf"
BLOOM_FILTERNAME="bf"

#CUCKOO_FILTERSIZE=4000000000
#CUCKOO_FILTERSIZE=536870900 #no of elements , indirectly uses 512MB memory
#CUCKOO_FILTERSIZE=41943040 #40*1024*1024. for 4GB it is 4294967296
#BLOOM_FILTERSIZE=41943040 #40*1024*1024
CUCKOO_FILTERSIZE=536870900 #40*1024*1024. for 4GB it is 4294967296
BLOOM_FILTERSIZE=536870900 #40*1024*1024
BLOOM_HASH_FNS=20
...
getCmdOutput ~/Desktop/Redis/redis/src/redis-cli cuckoocreate $CUCKOO_FILTERNAME $CUCKOO_FILTERSIZE
...
getCmdOutput "~/Desktop/Redis/redis/src/redis-cli bfcreate $BLOOM_FILTERNAME $BLOOM_FILTERSIZE $BLOOM_HASH_FNS" #bloom filter hash fns count..
#read -p "Press any key to continue"
...
getCmdOutput "~/Desktop/Redis/redis/src/redis-cli cuckooinsertelement $CUCKOO_FILTERNAME $VALUE"
#read -p "Press any key to continue"

echo "Initial test for insertion for bloom : "
getCmdOutput "~/Desktop/Redis/redis/src/redis-cli bfadd $BLOOM_FILTERNAME $VALUE"
#read -p "Press any key to continue"

sleep_before_reconnect=0.25;
cuckoo_insert_time=0;
bloom_insert_time=0;
while [ $ELEM_INSERTION_COUNT -le 536870800 ] #524288 = 512KB
...
do
	...
	if [ 0 == `expr $ELEM_INSERTION_COUNT % 1000000` ]
...
getCmdOutput "~/Desktop/Redis/redis/src/redis-cli cuckoocheckelement $CUCKOO_FILTERNAME $VALUE"
#read -p "Press any key to continue"

echo "Initial test for Element existence for bloom : "
getCmdOutput "~/Desktop/Redis/redis/src/redis-cli bfmatch $BLOOM_FILTERNAME $VALUE"
#read -p "Press any key to continue"

cuckoo_check_time=0;
bloom_check_time=0;
while [ $ELEM_CHECK_COUNT -le 536870800 ]
...
do
	...
	if [ 0 == `expr $ELEM_CHECK_COUNT % 1000000` ]
